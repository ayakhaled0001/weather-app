...
function Main({ weather }) {
  ...
  if (!weather) return <section>loading...</section>;
  const daysWithHours = {};
  ...
  return (
    <main className="grid grid-rows-[300px_160px_230px] grid-cols-[2fr_1fr] w-7/8 mx-auto gap-3 pb-8">
      {/* hero section */}
      <Hero country={country} weather={weather} getIcon={getIcon} />
      {/* Side bar */}
      <SideBar
        weather={weather}
        getIcon={getIcon}
      ...
      />
      <Properties hoursOfDay={hoursOfDay} days={days} />
      <DailyForecast weather={weather} getIcon={getIcon} />
    </main>
  );
}
export default Main;
